...
public class ComposterBlock extends Block implements ISidedInventoryProvider {
	...
	private static final VoxelShape[] SHAPE = Util.make(new VoxelShape[9], (p_220291_0_) -> {
		for (int i = 0; i < 8; ++i) {
			p_220291_0_[i] = VoxelShapes.combineAndSimplify(OUT_SHAPE, Block.makeCuboidShape(2.0D, Math.max(2, 1 + i * 2), 2.0D, 14.0D, 16.0D, 14.0D), IBooleanFunction.ONLY_FIRST);
		}
