...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
	...
	public void getMouseOver(float partialTicks) {
		...
		if (entity != null) {
			if (this.mc.world != null) {
				...
				EntityRayTraceResult entityraytraceresult = ProjectileHelper.rayTraceEntities(entity, vector3d, vector3d2, axisalignedbb, (p_215312_0_) -> !p_215312_0_.isSpectator() && p_215312_0_.canBeCollidedWith(), d1);
	...
	private void renderHand(MatrixStack matrixStackIn, ActiveRenderInfo activeRenderInfoIn, float partialTicks) {
		...
		if (this.mc.gameSettings.getPointOfView().isFirstPerson() && !flag && !this.mc.gameSettings.hideGUI && this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR) {
		...
		matrixStackIn.pop();
		if (this.mc.gameSettings.getPointOfView().isFirstPerson() && !flag) {
	...
	public void renderWorld(float partialTicks, long finishTimeNano, MatrixStack matrixStackIn) {
		...
		activerenderinfo.update(this.mc.world, this.mc.getRenderViewEntity() == null ? this.mc.player : this.mc.getRenderViewEntity(), !this.mc.gameSettings.getPointOfView().isFirstPerson(), this.mc.gameSettings.getPointOfView().func_243193_b(), partialTicks);
