...
public class Vector3d implements IPosition {
   ...
   public Vector3d linearInterpolate(Vector3d target, float alpha) {
      return new Vector3d(x + (alpha * (target.x - x)), y + (alpha * (target.y - y)), z + (alpha * (target.z - z)));
   }

   public Vector3d slerp(Vector3d target, float alpha) {
      double dot = dot(target);
      if (dot > 0.9995 || dot < -0.9995) return linearInterpolate(target, alpha);

      float theta0 = (float)Math.acos(dot);
      float theta = theta0 * alpha;

      float st = (float)Math.sin(theta);
      double tx = target.x - x * dot;
      double ty = target.y - y * dot;
      double tz = target.z - z * dot;
