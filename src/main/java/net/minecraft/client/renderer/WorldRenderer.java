...
public class WorldRenderer implements IResourceManagerReloadListener, AutoCloseable {
	...
	public static void drawVoxelShapeParts(MatrixStack matrixStackIn, IVertexBuilder bufferIn, VoxelShape shapeIn, double xIn, double yIn, double zIn) {
				...
				for (int k1 = i - l; k1 <= i + l; ++k1) {
					...
					if (biome.getPrecipitation() != Biome.RainType.NONE) {
                       ...
                       int l2 = Math.max(i2, j);
						...
						if (j2 != k2) {
							Random random = new Random(k1 * k1 * 3121L + k1 * 45238971L ^ j1 * j1 * 418711L + j1 * 13761L);
	...
	private void setupTerrain(ActiveRenderInfo activeRenderInfoIn, ClippingHelper camera, boolean debugCamera, int frameCount, boolean playerSpectator) {
		...
		if (!debugCamera && this.displayListEntitiesDirty) {
			...
			if (chunkrenderdispatcher$chunkrender != null) {
				...
				queue.add(new WorldRenderer.LocalRenderInformationContainer(chunkrenderdispatcher$chunkrender, null, 0));
			} else {
				...
				for (int i1 = -this.renderDistanceChunks; i1 <= this.renderDistanceChunks; ++i1) {
					for (int j1 = -this.renderDistanceChunks; j1 <= this.renderDistanceChunks; ++j1) {
						...
						if (chunkrenderdispatcher$chunkrender1 != null && camera.isBoundingBoxInFrustum(chunkrenderdispatcher$chunkrender1.boundingBox)) {
							...
							list.add(new WorldRenderer.LocalRenderInformationContainer(chunkrenderdispatcher$chunkrender1, null, 0));
						}
					}
				}
		...
		if (this.cloudOption == CloudOption.FANCY) {
			...
			for (int l1 = -32; l1 < 32; l1 += 32) {
				for (int i2 = -32; i2 < 32; i2 += 32) {
					bufferIn.pos(l1 + 0, f17, i2 + 32).tex((float) (l1 + 0) * 0.00390625F + f3, (float) (i2 + 32) * 0.00390625F + f4).color(f5, f6, f7, 0.8F).normal(0.0F, -1.0F, 0.0F).endVertex();
					...
					bufferIn.pos(l1 + 32, f17, i2 + 0).tex((float) (l1 + 32) * 0.00390625F + f3, (float) (i2 + 0) * 0.00390625F + f4).color(f5, f6, f7, 0.8F).normal(0.0F, -1.0F, 0.0F).endVertex();
					bufferIn.pos(l1 + 0, f17, i2 + 0).tex((float) (l1 + 0) * 0.00390625F + f3, (float) (i2 + 0) * 0.00390625F + f4).color(f5, f6, f7, 0.8F).normal(0.0F, -1.0F, 0.0F).endVertex();
				}
			}
		}
