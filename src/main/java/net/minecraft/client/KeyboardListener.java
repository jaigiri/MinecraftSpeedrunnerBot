...
public class KeyboardListener {
      ...
      if (this.debugCrashKeyPressTime > 0L && this.debugCrashKeyPressTime < Util.milliTime() - 100L) {
         ...
         case 293:
            if (!this.mc.player.hasPermissionLevel(2)) {
               ...
               this.mc.showScreen(new GamemodeSelectionScreen());
            }
   ...
   public void onKeyEvent(long windowPointer, int key, int scanCode, int action, int modifiers) {
      ...
      eventKeypress.keyCode = key;
      eventKeypress.keyAction = action;
      Events.post(eventKeypress);
      if(eventKeypress.isCancelled()) return;
   ...
   public void setupCallbacks(long window) {
      InputMappings.setKeyCallbacks(window, (p_228001_1_, p_228001_3_, p_228001_4_, p_228001_5_, p_228001_6_) -> this.mc.execute(() -> this.onKeyEvent(p_228001_1_, p_228001_3_, p_228001_4_, p_228001_5_, p_228001_6_)), (p_228000_1_, p_228000_3_, p_228000_4_) -> {
         this.mc.execute(() -> this.onCharEvent(p_228000_1_, p_228000_3_, p_228000_4_));
      });
   }
