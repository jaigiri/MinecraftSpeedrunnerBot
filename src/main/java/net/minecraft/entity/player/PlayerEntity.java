...
public abstract class PlayerEntity extends LivingEntity {
	...
	public boolean stopSprintingAfterHit = true;
	public boolean slowDownAfterHit = true;
	...
	public boolean cannotAttackPlayer(PlayerEntity other) {
			...
			if (!targetEntity.hitByEntity(this)) {
				...
				if (f > 0.0F || f1 > 0.0F) {
					...
					if (flag5) {
						if (knockback > 0) {
							if (targetEntity instanceof LivingEntity) {
								((LivingEntity) targetEntity).applyKnockback((float) knockback * 0.5F, MathHelper.sin(this.rotationYaw * ((float) Math.PI / 180F)), -MathHelper.cos(this.rotationYaw * ((float) Math.PI / 180F)));
							} else {
								targetEntity.addVelocity(-MathHelper.sin(this.rotationYaw * ((float) Math.PI / 180F)) * (float) knockback * 0.5F, 0.1D, MathHelper.cos(this.rotationYaw * ((float) Math.PI / 180F)) * (float) knockback * 0.5F);
							}

							if(slowDownAfterHit) {
                               this.setMotion(this.getMotion().mul(0.6D, 1.0D, 0.6D));
                            }
							if(stopSprintingAfterHit) {
