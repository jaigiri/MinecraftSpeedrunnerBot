...
public abstract class Entity implements INameable, ICommandSource {
	...
	private static ITextComponent func_233573_b_(ITextComponent name) {
		IFormattableTextComponent iformattabletextcomponent = name.copyRaw().setStyle(name.getStyle().setClickEvent(null));
	...
	public double getDistanceSq(Vector3d vec) {
		return getDistanceSq(vec.x, vec.y, vec.z);
	}
	...
	public void lookAtInterpolate(EntityAnchorArgument.Type anchor, Vector3d target, float amount) {
		Vector3d vector3d = anchor.apply(this);
		double d0 = target.x - vector3d.x;
		double d1 = target.y - vector3d.y;
		double d2 = target.z - vector3d.z;
		double d3 = MathHelper.sqrt(d0 * d0 + d2 * d2);
		this.rotationPitch = MathHelper.interpolateAngle(amount, rotationPitch, MathHelper.wrapDegrees((float) (-(MathHelper.atan2(d1, d3) * (double) (180F / (float) Math.PI)))));
		this.rotationYaw = MathHelper.interpolateAngle(amount, rotationYaw, MathHelper.wrapDegrees((float) (MathHelper.atan2(d2, d0) * (double) (180F / (float) Math.PI)) - 90.0F));
		this.setRotationYawHead(this.rotationYaw);
		this.prevRotationPitch = this.rotationPitch;
		this.prevRotationYaw = this.rotationYaw;
	}
