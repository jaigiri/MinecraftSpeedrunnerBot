...
public class ClientPlayNetHandler implements IClientPlayNetHandler {
   ...
   public void handleJoinGame(SJoinGamePacket packetIn) {
      ...
      this.client.showScreen(new DownloadTerrainScreen());
   ...
   public void handlePlayerPosLook(SPlayerPositionLookPacket packetIn) {
      ...
      if (!this.doneLoadingTerrain) {
         ...
         this.client.showScreen((Screen)null);
      }
   ...
   public void onDisconnect(ITextComponent reason) {
      ...
      if (this.guiScreenServer != null) {
         if (this.guiScreenServer instanceof RealmsScreen) {
            this.client.showScreen(new DisconnectedRealmsScreen(this.guiScreenServer, field_243491_b, reason));
         } else {
            this.client.showScreen(new DisconnectedScreen(this.guiScreenServer, field_243491_b, reason));
         }
      } else {
         this.client.showScreen(new DisconnectedScreen(new MultiplayerScreen(new MainMenuScreen()), field_243491_b, reason));
      }
   ...
   public void handleOpenHorseWindow(SOpenHorseWindowPacket packetIn) {
      ...
      if (entity instanceof AbstractHorseEntity) {
         ...
         this.client.showScreen(new HorseInventoryScreen(horseinventorycontainer, clientplayerentity.inventory, abstracthorseentity));
      }
   ...
   public void handleChangeGameState(SChangeGameStatePacket packetIn) {
      ...
      if (schangegamestatepacket$state == SChangeGameStatePacket.field_241764_a_) {
         ...
         if (i == 0) {
            ...
            this.client.showScreen(new WinGameScreen(true, () -> {
               this.client.player.connection.sendPacket(new CClientStatusPacket(CClientStatusPacket.State.PERFORM_RESPAWN));
            }));
         }
         ...
         if (f == 0.0F) {
            this.client.showScreen(new DemoScreen());
         } else if (f == 101.0F) {
      ...
      if (packetIn.eventType == SCombatPacket.Event.ENTITY_DIED) {
         ...
         if (entity == this.client.player) {
            if (this.client.player.isShowDeathScreen()) {
               this.client.showScreen(new DeathScreen(packetIn.deathMessage, this.world.getWorldInfo().isHardcore()));
            } else {
      ...
      if (this.validateResourcePackUrl(s)) {
         if (s.startsWith("level://")) {
            ...
            if (serverdata != null && serverdata.getResourceMode() == ServerData.ServerResourceMode.ENABLED) {
               ...
               this.client.execute(() -> {
                  this.client.showScreen(new ConfirmScreen((p_217274_3_) -> {
   ...
   public void handleOpenBookPacket(SOpenBookWindowPacket packetIn) {
      ...
      if (itemstack.getItem() == Items.WRITTEN_BOOK) {
         this.client.showScreen(new ReadBookScreen(new ReadBookScreen.WrittenBookInfo(itemstack)));
      }
